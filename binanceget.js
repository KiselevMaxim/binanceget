...
// Core function getting data from binance, parsing it and writing to XLS file
const getBinanceDataCSV = async (tF, tickPair, binanceTF, batches, limit) => {
    let tickPairTf = tickPair + '_' + tF
    let startTime = 1502942400000
    let endTime   = Date.now()
    let stringToWrite
    var ws = fs.createWriteStream(tickPairTf+'.csv', {flags: 'a' /* appending */})
    ws.write('tickPairTf,tickKey,O,C,bit0UP' + '\r\n')
    for (index of it(batches)) {
        try {
            let res = await ReqLinesSTR(tF, tickPair, binanceTF, startTime, endTime, limit)
            if (isNaN(res.startTime)) {
                startTime = endTime
                stringToWrite = res.res.join("")
                ws.write(stringToWrite)
                //console.log('startTime = ', startTime, 'index= ', index, 'stringToWrite=', stringToWrite)
                break
            }
            startTime = res.startTime/100000
            stringToWrite = res.res.join("")
    ...
    ws.end()
    console.log(tickPairTf+'.csv file done.')
}

// Parse data from binance response and returns formated result
const ReqLinesSTR = async (tF, tickPair, binanceTF, startTime, endTime, limit) => {
    let tickPairTf = tickPair + '_' + tF
    let resArr = []
    let startTimeres
...
getBinanceDataCSV('M1','BTCUSDT', '1m', 1600 /*batches*/ , 1000 /*records in batch*/) // get data by batches, set records per one batch. Batches divided by delay 0.9 sec